...
public class RestAdapterData {
    public String getAegitekFarmerCollection(String unitCodeFromPage, String empCode) {
        ...
        if (unitCodeFromPage == null || unitCodeFromPage.trim().isEmpty() || empCode == null ||
            ...
            try {
                ...
                ResultSet rs2 =
                    stmt3.executeQuery("SELECT * FROM API_Authorisation WHERE STATUS='Y' AND AUTH_TYPE='TRANSACTION'");
        ...
        try {
            ...
            String jsonInputString =
                "{\"limit\": 0,\"offset\": 0,\"search\": \"\",\"filters\": {\"unitCode\": \"" + unitCodeFromPage +
                "\",\"additionalProp2\": \"string\",\"additionalProp3\": \"string\"},\"sort\": {\"additionalProp1\": \"string\",\"additionalProp2\": \"string\",\"additionalProp3\": \"string\"}}";
                    ...
                    for (int i = 0; i < responseData.length(); i++) {
                        ...
                        try {
                            insertDetailsQuery =
                                ...
                                "','" + chillingCode + "',null,'" + formattedDate.toString() + "','" + time + "','" +
                                milkType + "','" + localCode + "','" + extendedCode + "','" + quantity + "','" + fat +
                                "','" + snf + "','" + amount + "','" + quantity_Mode + "','" + measurement_Mode +
                                "','" + shift + "','" + rate + "','" + itemCode + "'," + "'0','0','E','" + empCode +
                                "',SYSDATE,'" + empCode + "',SYSDATE,'" + ApiRefno + "','Y','" + primeryId + "','" +
                                can + "','" + route_Code + "')";
    ...
    public String getAegiteckRMRDCollectionData(String unitCodeFromPage, String empCode) {
        Connection conn = null;
        Statement stmt = null;
        Statement stmt2 = null;
        Statement stmt3 = null;
        ...
        try {
            URL url = new URL("https://betaservices.milkmatrix.com/api/v1/Milk/list-rmrd-custom");
            HttpURLConnection con = (HttpURLConnection) url.openConnection();
            con.setRequestMethod("POST");
            con.setRequestProperty("Content-Type", "application/json");
            con.setRequestProperty("Accept", "application/json");
                ...
                if (count > 0) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < responseData.length(); i++) {
                            JSONObject jsonobjectDtl = responseData.getJSONObject(i);
                            String societyCode = null;
                            Object obj = jsonobjectDtl.opt("societyCode");
                            if (obj != null && obj != JSONObject.NULL) {
                                societyCode = obj.toString().trim();
                            }
                            System.out.println("societyCode: " + societyCode);
                            String eDate = null;
                            if (jsonobjectDtl.getString("eDate").toString() == null ||
                            ...
                            //                            }
                            Object obj_sampleid = jsonobjectDtl.opt("sampleid");
                            if (obj_sampleid != null && obj_sampleid != JSONObject.NULL) {
                                sampleid = obj_sampleid.toString().trim();
                            }
                            ...
                            try {
                                ...
                                //                                    "VALUES('" + unitCode + "','" + uploadid + "',GLOBAL_OCI_SEQ.nextval,'" +
                                //                                    chillingCode + "','" + chillingCode + "','" +TO_DATE(formattedDate,'DD-MON-YYYY') + "','" +
                                //                                    time + "','" + milkType + "','" + localCode + "','" + extendedCode + "','" +
                                //                                    quantity + "','" + fat + "','" + snf + "','" + amount + "','" + quantity_Mode +
                                //                                    "','" + measurement_Mode + "','" + shift + "','" + rate + "','" + itemCode + "'," +
                                ...
                                System.out.println("shift = [" + shift + "]");
                                insertDetailsQuery =
                                    "INSERT INTO mm_rmrd_data_upload_api (" +
                                    "UNIT_CD,UPLOAD_ID,UPLOAD_LINE_ID,CP_CODE,CP_VENDOR,E_DATE,E_TIME," +
                                    "MILK_TYPE,LOCAL_CODE,EXTENDED_CODE,QUANTITY,FAT,SNF,AMOUNT," +
                                    "QUANTITY_MODE,MEASUREMENT_MODE,SHIFT,RATE,ITEM_CD,SYS_RATE_ID," +
                                    "SYS_RATE,REC_STATUS,CREATED_BY,CREATED_DATE,MODIFY_BY,MODIFY_DATE," +
                                    "API_REFNO,API_FLAG,NR,ROUTE_CODE,RECIVE_CAN,ACCEPT_CAN,SAMPLE_ID," +
                                    "REJECT_CAN,REJECT_QTY,ADULTERATION_STATUS,REMARK) " + "VALUES('" + unitCode +
                                    "','" + uploadid + "',GLOBAL_OCI_SEQ.NEXTVAL,'" + chillingCode + "','" +
                                    chillingCode + "'," + "TO_DATE('" + formattedDate + "','DD-MON-YYYY'),'" + time +
                                    "','" + milkType + "','" + localCode + "','" + extendedCode + "','" + quantity +
                                    "','" + fat + "','" + snf + "','" + amount + "','" + quantity_Mode + "','" +
                                    measurement_Mode + "','" + shift + "','" + rate + "','" + itemCode +
                                    "','0','0','E','" + empCode + "',SYSDATE,'Admin',SYSDATE,'" + ApiRefno + "','Y','" +
                                    primeryId + "','" + route_Code + "','" + rcans + "','" + acans + "','" + sampleid +
                                    "',NULL," + rejecT_QTY + ",'" + adulteration_STATUS + "','" + remark + "')";
